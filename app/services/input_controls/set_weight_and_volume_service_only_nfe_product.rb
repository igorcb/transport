module InputControls
  class SetWeightAndVolumeServiceOnlyNfeProduct
    ...
    def call
      #byebug
      weight = @input_control.nfe_xmls.where(equipamento: NfeXml::TipoEquipamento::NOTA_FISCAL).sum("COALESCE(peso,0)")
      volume = @input_control.nfe_xmls.where(equipamento: NfeXml::TipoEquipamento::NOTA_FISCAL).sum("COALESCE(volume,0)")
